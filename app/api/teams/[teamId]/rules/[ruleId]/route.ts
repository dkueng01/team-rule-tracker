...
export async function PUT(req: NextRequest, { params }: { params: { teamId: string; ruleId: string } }) {
...
export async function DELETE(_: NextRequest, { params }: { params: { teamId: string; ruleId: string } }) {
    const user = await stackServerApp.getUser();
    if (!user?.id) return NextResponse.json({ error: "Unauthorized" }, { status: 401 })
